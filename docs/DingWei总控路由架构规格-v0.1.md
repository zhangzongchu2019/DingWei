...
```json
{
  ...
  "owner_key": "owner1",
  "online_sessions": [
    {"session":"example-developer","tool":"TOOL_A","model":"model-a","role":"dev","busy":false},
    {"session":"example-tester","tool":"TOOL_B","model":"model-b","role":"qa","busy":false},
    {"session":"example-coordinator","tool":"TOOL_C","model":"model-c","role":"coordinator","busy":true}
  ],
...
```json
{
  ...
  "targets": [
    {"session": "example-tester", "instruction": "复测登录全流程，回执用 #发起会话名 开头"}
  ],
  "subtasks": [
    {"session": "example-developer", "instruction": "自查登录接口最近改动"},
    {"session": "example-tester", "instruction": "端到端复测登录并记录异常"}
  ],
  "confidence": 0.0
}
```
