...
> 版本 v0.24 · 通用工程版(Go实现+脚手架; Docker交付; 全量不分期; 容量≤100人·≤200写每秒基线, 异步批量写可上探数千/秒; 敏感不入git) · 面向「据此生成工程代码」编写
...
>
> **v0.24 变更（异步批量写入）**：新增 §13.2.1——把 §13.1 的单写者细化为**有界队列（默认 5 万条）+ 单写入器批量事务（N=200 或 T=20ms）+ 满时阻塞背压（不丢，正常几乎不触发）+ 批量提交不逐条 fsync（崩溃/写失败丢在途批为可接受损失）+ M9 写配置同步旁路**；写锁零争用 + 摊薄 fsync，容量上探数千写/秒、数百人，仍单文件 SQLite。
...
- **后端可插拔**：**本规模（≤200 写/秒）用 SQLite 表队列即可**（message 表 + 状态机），无需引入 Redis；Redis Stream 仅在规模数量级增长时可选。抽象为 `Queue` 接口（见 §13.2）。
...
| **聚合项目周报** | 草稿 `0 22 * * 0`（周日 22:00）；发布 `0 2 * * 1`（周一 02:00） | 草稿周日 15:00；发布周日 19:00 | 草稿周一 06:00；发布周一 10:00 | 汇总 + 审批 + 发布 |
...
- 审阅人、DM bot、发布群与发布 bot 均从聚合项目自身 `owner_key` / `notify_chat_id` / `notify_bot_id` 派生；代码不得写死任何具体成员、群或机器人。
- 聚合草稿/定稿复用 `project_weekly_report`，以 `status`/`approved_at`/`vetoed_at`/`published_at` 记录审阅与发布状态。
...
-- 项目周报；非聚合项目写 final，聚合项目写 draft/approved/vetoed/published
project_weekly_report(id, project_id, week, content, status,
                      approved_at, vetoed_at, published_at,
...
- 性能：**容量基线 ≤100 人 / 峰值写入 ≤200 次每秒**（见 §13.2）；此规模 SQLite WAL 足够，不需 Redis/PG/水平扩展；LLM 抽取增量处理 + 长会话采样。
...
- **容量假设（设计基线）**：小团队应用，**≤ 100 人**；消息低频（**不会出现单人 1 条/秒**），**峰值总写入 ≤ 200 次/秒**。
- **结论**：此 QPS 下 SQLite（**WAL + busy_timeout**）完全够用，**单文件即长期方案，不存在写瓶颈**（SQLite WAL 可达上千写/秒，200/s 余量极大）。前述评审的"SQLite 天花板/必须上 Redis/PG/水平扩展"在本规模**不成立**。
- **并发**：WAL 下读写并发良好；一个轻量串行写入口即可（200/s 无压力）；归档/删除用独立连接、分批、不阻塞主写。
...
### 13.2.1 异步批量写入（提升写并发与容量上限）
> 把 §13.1 的"单写者 goroutine"细化为**异步批量写入**：在容量基线（≤200 写/秒）之上平滑支撑**数千写/秒、数百人**，仍单文件 SQLite，不引入 Redis/PG。

- **写队列**：一个**有界内存 channel**（默认容量 **5 万条，可配**）；所有写封装为写请求投入队列，**生产者入队即返回**（不阻塞在磁盘 I/O 上，故延迟低、支撑更多在线人）。
- **单写入器**：唯一消费者 goroutine，**累计取批（默认 N=200 条 或 T=20ms，先到者触发 flush）→ 单事务批量执行 → 提交**；全进程仅它写库，**写锁零争用** + 批量摊薄 fsync → SQLite（WAL）可达数千写/秒。
- **读**：走独立连接（WAL 读写并发良好），不经写队列。
- **满时策略 = 阻塞生产者（背压）**：队列满则生产者等待空位（表现为"写入暂时变慢"，**不丢数据**）；因队列大、写入器快，**正常运行几乎不触发**，仅作极端持续过载时的安全阀。
- **持久性（可接受损失，2026-07-06 定调）**：批量提交、**不逐条 fsync**；进程崩溃丢"已入队未落盘"的在途批、文件写失败丢该批 → 均为**可接受的环境故障**（记日志告警，不做强持久保证）。
- **同步旁路**：**M9 写配置**等"须立即可见/须确认"的少量写走同步落盘、不进队列；其余全异步。
- 经 §13.1 统一写入口 + `Queue`/Repository 接口封装，可测（fake 队列 + fake clock 时间旅行）。
