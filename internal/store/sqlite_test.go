...
func TestSessionDeliveryClaimBypassesExpiredDeliveredAndIsMonotonic(t *testing.T) {
	db, ctx := newTestSQLite(t)
	now := time.Now().UTC()
	for i, id := range []string{"dlv_1", "dlv_2"} {
		if err := db.EnqueueSessionDelivery(ctx, model.SessionDelivery{
			ID: id, KeyID: "key", SessionName: "home", Envelope: `{"body":"x"}`,
			SourceMessageID: "msg_" + id, SourceType: "feishu_inbound", CreatedAt: now.Add(time.Duration(i) * time.Millisecond),
		}); err != nil {
			t.Fatalf("enqueue %s: %v", id, err)
		}
	}
	first, err := db.ClaimSessionDelivery(ctx, "key", "home", now.Add(time.Second))
	if err != nil || first == nil || first.ID != "dlv_1" {
		t.Fatalf("first claim = %+v, %v", first, err)
	}
	if err := db.UpdateSessionDelivery(ctx, first.ID, "delivered", "", now.Add(-time.Second)); err != nil {
		t.Fatal(err)
	}
	second, err := db.ClaimSessionDelivery(ctx, "key", "home", now.Add(time.Second))
	if err != nil || second == nil || second.ID != "dlv_2" {
		t.Fatalf("expired delivered must not starve FIFO: %+v, %v", second, err)
	}
	if err := db.UpdateSessionDelivery(ctx, first.ID, "processed", "", now); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateSessionDelivery(ctx, first.ID, "delivered", "", now); err != nil {
		t.Fatal(err)
	}
	got, err := db.GetSessionDelivery(ctx, first.ID)
	if err != nil || got.State != "processed" || got.ProcessedAt == nil {
		t.Fatalf("state regressed: %+v, %v", got, err)
	}
}

func TestSessionDeliveryBusyUpdateDoesNotConsumeAttempts(t *testing.T) {
	db, ctx := newTestSQLite(t)
	now := time.Now().UTC()
	if err := db.EnqueueSessionDelivery(ctx, model.SessionDelivery{
		ID: "dlv_busy", KeyID: "key", SessionName: "home", Envelope: `{"body":"x"}`,
		SourceMessageID: "msg_busy", SourceType: "feishu_inbound", CreatedAt: now,
	}); err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 15; i++ {
		claimed, err := db.ClaimSessionDelivery(ctx, "key", "home", now.Add(time.Duration(i)*time.Minute))
		if err != nil || claimed == nil || claimed.ID != "dlv_busy" {
			t.Fatalf("claim %d = %+v, %v", i, claimed, err)
		}
		if claimed.Attempts != 1 {
			t.Fatalf("busy claim attempts=%d, want transient 1", claimed.Attempts)
		}
		if err := db.UpdateSessionDeliveryBusy(ctx, claimed.ID, "cli_busy", now.Add(time.Duration(i+1)*time.Minute)); err != nil {
			t.Fatal(err)
		}
		got, err := db.GetSessionDelivery(ctx, "dlv_busy")
		if err != nil || got == nil {
			t.Fatalf("get after busy: %+v %v", got, err)
		}
		if got.State != "queued" || got.Attempts != 0 {
			t.Fatalf("after busy state=%s attempts=%d, want queued attempts 0", got.State, got.Attempts)
		}
	}
}
...
func TestMigrateUpgradesExistingDBWithSeenPersonAndBotSecret(t *testing.T) {
  ...
  active      INTEGER NOT NULL DEFAULT 1
	...
	if !columnExists(t, db, "session_endpoint", "version") {
		t.Fatal("session_endpoint.version missing after migration")
	}
...
func TestUpsertSessionEndpointPreservesMirrorToWhenReconnectOmitsIt(t *testing.T) {
	db, ctx := newTestSQLite(t)
	mirrorTo := "ou_X#FB-Y#ExampleConnector"
...
func TestUpsertSessionEndpointPreservesOwnerAndFullNameWhenReconnectOmitsThem(t *testing.T) {
	db, ctx := newTestSQLite(t)
	if err := db.UpsertSessionEndpoint(ctx, model.SessionEndpoint{KeyID: "FB-test", SessionName: "developer", FullSessionName: "alice-developer-test", OwnerKey: "alice", Active: true}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpsertSessionEndpoint(ctx, model.SessionEndpoint{KeyID: "FB-test", SessionName: "developer", FullSessionName: "", OwnerKey: "", Active: true}); err != nil {
		t.Fatal(err)
	}
	endpoints, err := db.ListSessionEndpoints(ctx)
	if err != nil || len(endpoints) != 1 {
	...
	if endpoints[0].OwnerKey != "alice" || endpoints[0].FullSessionName != "alice-developer-test" {
		t.Fatalf("identity metadata overwritten: %+v", endpoints[0])
	}
}
...
func TestMessageIngressProvenanceDefaultsUntrustedAndSurvivesRetry(t *testing.T) {
	db, ctx := newTestSQLite(t)
	if err := db.EnqueueMessage(ctx, model.Message{
		ID: "direct", ChatEntityID: "chat1", Direction: model.DirectionIn,
		BotChannelID: "bot1", ChatType: model.ChatPersonal, Content: `{"text":"direct"}`,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if msg.IngressProvenance != model.IngressUntrusted {
		t.Fatalf("direct provenance = %q", msg.IngressProvenance)
	}
	if err := db.FailMessage(ctx, msg.ID, "retry"); err != nil {
		t.Fatal(err)
	}
	msg, err = db.ClaimNextMessage(ctx, model.DirectionIn)
	if err != nil || msg == nil || msg.IngressProvenance != model.IngressUntrusted {
		t.Fatalf("retry upgraded provenance: msg=%+v err=%v", msg, err)
	}
}

func TestLegacyMessageRowDefaultsUntrusted(t *testing.T) {
	db, ctx := newTestSQLite(t)
	_, err := db.db.ExecContext(ctx, `INSERT INTO message
		(id,chat_entity_id,direction,bot_channel_id,chat_type,sender_open_id,content_json,status,attempts,created_at)
		VALUES('legacy','chat1','in','bot1','personal','','{}','queued',0,?)`, time.Now().UTC().Format(time.RFC3339))
	if err != nil {
		t.Fatal(err)
	}
	msg, err := db.ClaimNextMessage(ctx, model.DirectionIn)
	if err != nil || msg == nil || msg.IngressProvenance != model.IngressUntrusted {
		t.Fatalf("legacy provenance: msg=%+v err=%v", msg, err)
	}
}

func TestMessageIngressProvenanceRejectsUnknownValue(t *testing.T) {
	db, ctx := newTestSQLite(t)
	if err := db.EnqueueMessage(ctx, model.Message{
		ID: "unknown", ChatEntityID: "chat1", Direction: model.DirectionIn,
		BotChannelID: "bot1", ChatType: model.ChatPersonal,
		IngressProvenance: model.IngressProvenance("payload_verified"), Content: `{}`,
	}); err != nil {
		t.Fatal(err)
	}
	msg, err := db.ClaimNextMessage(ctx, model.DirectionIn)
	if err != nil || msg == nil || msg.IngressProvenance != model.IngressUntrusted {
		t.Fatalf("unknown provenance: msg=%+v err=%v", msg, err)
	}
}
...
func TestEnqueueMessageRedactsContentBeforeStorage(t *testing.T) {
	...
	if err := db.EnqueueMessage(ctx, model.Message{
		...
		FeishuMsgID:  "f1",
		ChatType:     model.ChatPersonal,
		Content:      `{"token":"abc123","text":"Bearer abc.def.ghi 手机 13800138000"}`,
	}); err != nil {
		t.Fatal(err)
	}
	msg, err := db.ClaimNextMessage(ctx, model.DirectionIn)
	...
	for _, leaked := range []string{"abc123", "abc.def.ghi", "13800138000"} {
...
func TestListAPIKeyAccountsForKeys(t *testing.T) {
	db, ctx := newTestSQLite(t)
	for _, pair := range [][2]string{
		{"FB-key-1", "dev:personal:ou_one"},
		{"FB-key-1", "dev:group:oc_one"},
		{"FB-key-2", "dev:personal:ou_two"},
	} {
		if err := db.BindAPIKeyAccount(ctx, pair[0], pair[1]); err != nil {
			t.Fatal(err)
		}
	}
	got, err := db.ListAPIKeyAccountsForKeys(ctx, []string{"FB-key-2", "FB-key-1", "FB-key-1", ""})
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(got["FB-key-1"], []string{"dev:group:oc_one", "dev:personal:ou_one"}) {
		t.Fatalf("key1 accounts=%+v", got["FB-key-1"])
	}
	if !reflect.DeepEqual(got["FB-key-2"], []string{"dev:personal:ou_two"}) {
		t.Fatalf("key2 accounts=%+v", got["FB-key-2"])
	}
	if _, ok := got[""]; ok {
		t.Fatalf("empty key should not be present: %+v", got)
	}
}
...
func TestControlPlaneP1QueueRulesRetryReaperAndStats(t *testing.T) {
	...
	task, inserted, err := db.EnqueueControlTask(ctx, model.ControlTask{
		ID:         "ct1",
		Source:     "feishu",
		SourceAddr: "ou_1#FB-test#ExampleBot",
		OwnerKey:   "u1",
		RawInput:   "hello",
		ExpireAt:   &expire,
	})
