...
func TestApproveKeyApplicationWithGrantCommitsCredentialAndGrantAtomically(t *testing.T) {
	...
	if err := db.ApproveKeyApplicationWithGrant(ctx, app.ID, "ou_approver",
		...
		model.ServiceAPIKey{ID: "FB-apply-test", ServiceID: "apply:ou-applicant", KeyHash: "hash", Label: "ou_applicant", Active: true, CreatedAt: now},
		...
		model.Message{ID: "grant-ka1", ChatEntityID: "dev:personal:ou_applicant", Direction: model.DirectionOut, BotChannelID: "dev", FeishuMsgID: "grant-ka1", ChatType: model.ChatPersonal, Content: `{"text":"key_id: FB-apply-test\nsecret 已隐藏"}`, Status: "queued"},
		now,
	); err != nil {
		t.Fatal(err)
	}
	got, err := db.GetKeyApplication(ctx, app.ID)
	if err != nil || got == nil || got.Status != "approved" || got.KeyID != "FB-apply-test" || got.ServiceID != "apply:ou-applicant" {
		t.Fatalf("application=%+v err=%v", got, err)
	}
	accounts, err := db.ListAPIKeyAccounts(ctx, "FB-apply-test")
