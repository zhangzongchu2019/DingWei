...
func TestAdminSessionMirrorPersists(t *testing.T) {
	...
	if !strings.Contains(page, "机器人账号") || strings.Contains(page, "ou_alice#FB-test#ExampleBot") {
		t.Fatalf("session page should hide raw mirror address before enabled: %s", page)
	}
	postForm(t, mux, "/admin/sessions", url.Values{"action": {"mirror_on"}, "key_id": {"FB-test"}, "session_name": {"home"}, "bot_name": {"ExampleBot"}})
	endpoints, err := db.ListSessionEndpoints(ctx)
	if err != nil || len(endpoints) != 1 || !endpoints[0].MirrorEnabled || endpoints[0].MirrorTo != "ou_alice#FB-test#ExampleBot" {
		t.Fatalf("mirror on endpoints=%+v err=%v", endpoints, err)
	}
	page = getAuth(t, mux, "/admin/sessions").Body.String()
	if !strings.Contains(page, "UserOne via ExampleBot") || strings.Contains(page, "ou_alice#FB-test#ExampleBot") {
...
func TestAdminSessionMirrorAdvancedAddressStillWorks(t *testing.T) {
	...
	postForm(t, mux, "/admin/sessions", url.Values{"action": {"mirror_on"}, "key_id": {"FB-test"}, "session_name": {"home"}, "mirror_to": {"oc_group#FB-test#ExampleBot"}})
	endpoints, err := db.ListSessionEndpoints(ctx)
	if err != nil || len(endpoints) != 1 || endpoints[0].MirrorTo != "oc_group#FB-test#ExampleBot" {
...
func TestAdminProjectsCRUDAndMemberAssignment(t *testing.T) {
	...
	postForm(t, mux, "/admin/projects", url.Values{"id": {"proj:test"}, "name": {"测试项目"}, "parent_id": {"proj:default"}, "owner_key": {"alice"}, "product_manager_key": {"bob"}, "notify_chat_id": {"oc_team"}, "notify_bot_id": {"ExampleBot"}, "active": {"1"}})
...
func TestAdminSchedulePasteRejectsInvalidAndAppendsWithRollback(t *testing.T) {
	srv, db, mux, ctx := newAdminTestServer(t)
	srv.sessions["tok"] = "admin"
	if err := db.UpsertProject(ctx, model.Project{ID: "proj:test", Name: "测试项目", NotifyBotID: "ExampleBot", Active: true}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpsertMember(ctx, model.Member{OwnerKey: "alice", DisplayName: "Alice", Role: model.RoleMember, Active: true}); err != nil {
		t.Fatal(err)
...
func TestAdminSessionsShowsToolModelAndFullSessionName(t *testing.T) {
	srv, db, mux, ctx := newAdminTestServer(t)
	srv.sessions["tok"] = "admin"
	if err := db.UpsertSessionEndpoint(ctx, model.SessionEndpoint{KeyID: "FB-test", SessionName: "developer", FullSessionName: "sh-developer-e0d10000", Tool: "CODEX", Model: "gpt-5.5", Producer: true, TargetGroup: "oc_ai", LastSeenAt: time.Now(), Active: true}); err != nil {
		t.Fatal(err)
	}
	page := getAuth(t, mux, "/admin/sessions").Body.String()
	for _, want := range []string{"<th>工具</th>", "<th>大模型</th>", "<th>完整会话名</th>", "<th>producer</th>", "<th>target_group</th>", "CODEX", "gpt-5.5", "sh-developer-e0d10000", "oc_ai"} {
		if !strings.Contains(page, want) {
...
// F2: update_self 的单调闸以 helper 在线上报版本(session_endpoint.version)为准，
// 不依赖 app_config 记账——这样失败下发不会烧号，成功升级重连即刷新。
func TestAdminProvisionUpdateSelfGatesOnLiveSessionVersion(t *testing.T) {
	srv, db, _, ctx := newAdminTestServer(t)
	if err := db.UpsertSessionEndpoint(ctx, model.SessionEndpoint{
		KeyID: "FB-test", SessionName: "dev1", OwnerKey: "alice",
		LastSeenAt: time.Now().UTC(), Active: true, Version: "0.1.2.6",
	}); err != nil {
		t.Fatal(err)
	}
	// 比在线版本低 → 拒
	if err := srv.ensureProvisionVersionMonotonic(ctx, "update_self", "FB-test", "dev1", "self", "0.1.2.5"); err == nil {
		t.Fatal("低于在线版本的 update_self 应被拒")
	}
	// 高于在线版本 → 放行
	if err := srv.ensureProvisionVersionMonotonic(ctx, "update_self", "FB-test", "dev1", "self", "0.1.2.7"); err != nil {
		t.Fatalf("高于在线版本应放行: %v", err)
	}
	// 一条陈旧的 app_config 记录不得影响 update_self（它只看在线版本）——
	// 复现 V1 的 bug：一次失败下发若曾写过高版本记录，也不能把合法下发挡住。
	srv.recordProvisionVersion(ctx, "update_self", "FB-test", "dev1", "self", "9.9.9")
	if err := srv.ensureProvisionVersionMonotonic(ctx, "update_self", "FB-test", "dev1", "self", "0.1.2.7"); err != nil {
		t.Fatalf("陈旧 app_config 记录不应影响 update_self 闸: %v", err)
	}
}

func TestAdminProvisionValidatesURLHost(t *testing.T) {
	t.Setenv("WP_PROVISION_ALLOWED_HOSTS", "")
	t.Setenv("SH_PROVISION_ALLOWED_HOSTS", "")
	defaultHosts := provisionAllowedHosts()
	if !defaultHosts["localhost"] {
		t.Fatal("default provision hosts must include localhost")
	}
	if defaultHosts["downloads.example.com"] {
		t.Fatal("default provision hosts must not include unconfigured hosts")
	}
	if err := validateProvisionURL("http://localhost/a.tar", defaultHosts); err != nil {
		t.Fatalf("default localhost should pass: %v", err)
	}
	if err := validateProvisionURL("https://downloads.example.com/a.tar", defaultHosts); err == nil {
		t.Fatal("unconfigured host should not be allowed by default")
	}
	t.Setenv("WP_PROVISION_ALLOWED_HOSTS", "downloads.example.com,localhost")
	if err := validateProvisionURL("https://downloads.example.com/a.tar", provisionAllowedHosts()); err != nil {
		t.Fatal(err)
	}
