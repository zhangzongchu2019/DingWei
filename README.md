...
> 完整设计见 [`docs/产品规范.md`](docs/产品规范.md)。本仓为 Go 实现的工程脚手架。
...
DingWei 最早只是一个**手搓的小玩具**——为验证“消息路由 + AI 会话协同”这个点子随手拼出来的原型：能跑，但谈不上架构。

本仓库是它**重新架构后的「工作组版本」**：从单人小工具，演进为面向**团队 / 工作组**的协同中枢——统一的寻址与路由、人 + Agent 网络协作、排期与进度佐证、后台管理，都在这一代被系统性地重新设计。

因此本仓库以**这一代架构的干净基线**为起点（单一提交）；早期原型的零散实验历史不在此延续。往后的演进（如跨用户 / 跨工作组协作）也将在这一基线上推进。
...
### 本地
```bash
export WP_ADMIN_INIT_PASSWORD='replace-me'
go mod tidy
make build && ./bin/workpulse # 默认 :8080；/healthz、/admin/login
```

## 配置与数据
- **bootstrap 走环境变量**：`WP_DB_PATH / WP_DATA_DIR / WP_ADDR / WP_ADMIN_USER / WP_ADMIN_INIT_PASSWORD`。
- **其余配置经 M9 后台写入 SQLite**；可配项结构见 `config/app.example.yaml`（仅占位，无真值）。
- 数据目录由 `WP_DATA_DIR` 设置：SQLite 主库 + 月度归档库。
...
## 状态
工程脚手架（接口/分层/SQLite/迁移/后台登录）；业务与平台细节按 `docs/产品规范.md` 逐模块实现。
