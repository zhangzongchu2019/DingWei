...
`sessionHelper` 是 WorkPulse / feishu-bot-bridge 的会话侧适配器。它连接
...
- 默认会静默采集本地 AI CLI 会话轮次到 WorkPulse，用于团队进度佐证；不会发到飞书。需关闭请联系管理员设置成员 `evidence_optout`，或本地把 `SH_COLLECT=0`。
...
| `SH_KEY_ID` | WorkPulse 公开租户标识，进入地址 |
| `SH_SECRET` | WorkPulse 私密连接 secret，仅用于 `Authorization: Bearer` |
...
| `SH_WS_BASE` | WorkPulse WebSocket base URL |
